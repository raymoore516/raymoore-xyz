...
- The first loss eliminates an entrant, except a Week 1 loss with an affirmative `Buyback` value. That exception permits continued participation, but the next loss is final. A first loss in Week 2 or later cannot use a buyback.
...
- On the actual loss week, show the entrant's pick (or check when hidden) in a red row. A Week 1 loss forgiven by buyback uses a lighter red instead.
...
- Row backgrounds: white for pending, green for survival, lighter red (`#FFF5F5`) for a Week 1 loss forgiven by buyback, red for this week's final loss, and light gray for prior elimination. Prior-elimination rows use non-italic text with regular-weight dark-gray names and lighter-gray elimination labels. Other contestant names remain bold. Accessible text also identifies result status.
...
- In revealed weeks, sort groups as survival, pending, this week's loss, then previously eliminated. The loss group combines buybacks and final eliminations so both red shades remain together. Sort the first three groups by NFL team code ascending, then contestant name ascending (both case-insensitive). Use the original team code before mapping it to its display name.
...
- Keep the bold week dropdown and compact table text. On mobile, keep the pick column wide enough to prevent its content from wrapping and let the contestant-name column use the remaining width, wrapping long names as needed. Retain horizontal overflow as a fallback for exceptional content.
...
- `WeekView.picksHidden` describes visibility. `ContestantView.status` is `PENDING`, `SURVIVAL`, `BUYBACK`, `ELIMINATION`, or `ELIMINATED`; `eliminationWeek` is nullable and represents final elimination as of that view.
...
- With the sandbox's Week 1 losses, Jordan (no buyback) has a red row and enters the graveyard in Week 2; Ray and Tyler (`Buyback = X`) have lighter-red rows and continue.
...
- Verify the revealed-week group order is green, white, both red shades together, then gray. Within each of the first three groups, verify team-code ordering and alphabetical contestant-name ordering for matching codes. Buybacks and final losses must interleave by team code and name rather than forming separate subgroups. Gray rows remain ordered by elimination week descending, then contestant name.
