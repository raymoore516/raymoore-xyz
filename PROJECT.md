...
Build a personal website as a monolithic web application in a monorepo. Its projects include [Madison SC](projects/madisonsc/README.md), an NFL pick'em league tracker, and [Survivor League](projects/survivor/README.md), a read-only Google Sheets view. A shared, responsive hamburger navigation bar is also part of the website. Future features may include an arcade or blog.

This is the implementation plan for AI coding agents. The foundation is implemented: Java 25, Spring Boot 4.1.1 with MVC, Maven Wrapper 3.9.11, local PostgreSQL 15.19 through Compose, Flyway V1, Spring Data JDBC `Contestant` and `Pick` records, public Madison SC APIs, secret-protected administrator pick submission, and a public Survivor League API backed by a periodically refreshed in-memory Google Sheets cache. The React/TypeScript frontend uses Vite, React Router, a responsive slide-out hamburger menu, a global home page, and project-specific views. Further features remain planned. It defines shared technologies, architecture, and authentication for the personal website. Project-specific business requirements and data models live in `projects/<project>/README.md`. Proposed API contracts and unresolved business rules are labeled explicitly.
...
- **Proof of concept:** public home page, shared desktop/mobile hamburger navigation, the Madison SC project, the read-only Survivor League Google Sheets view, administrator API writes using the `api-secret` HTTP header, Postgres persistence, and Flyway SQL migrations.
...
When the built frontend's `static/index.html` is present on the classpath, `BrowserRouterController` forwards the small, explicit set of React page routes to `index.html`, allowing React Router to handle direct navigation and refresh. Unknown URLs, missing assets, and missing API routes keep Spring's normal `404` behavior. Add each new React page route to both React Router and this production entry-point list. The controller is inactive in backend-only local builds; Vite handles local page navigation.

The single administrator submission controller binds its required `api-secret` header with Spring MVC's `@RequestHeader` and compares it with the configured secret before processing the submission.
...
│       │   │   └── HamburgerMenu.tsx    # Global navigation drawer
...
Use one `raymoorexyz` PostgreSQL database per environment. Each database-backed project gets its own schema within that database; for example, Madison SC owns `madisonsc`. Projects that do not persist data, such as Survivor League, do not need an empty schema. Project tables, constraints, and domain mappings are documented in that project's plan, starting with the [Madison SC database model in the Madison SC README](projects/madisonsc/README.md).
...
- The proof of concept validates the required header directly in the sole administrator submission controller. When Spring Security is introduced for browser login, migrate this check into that security architecture rather than maintaining two authentication systems. Do not generalize this single-endpoint mechanism or introduce an OAuth/JWT server for it.
...
Idempotent statements address SQL execution, but Flyway also needed to adopt the production database's nonempty schema without an existing history table. The completed first production attachment temporarily used `spring.flyway.baseline-on-migrate=true` with `spring.flyway.baseline-version=0`. That recorded a starting baseline below `V1`, allowing the idempotent `V1` to execute; the usual baseline version of `1` would have skipped it. The temporary settings have now been removed from `application.yml`, restoring the default of `false`. Subsequent starts use the existing history table, and fresh databases begin with `V1`. See [Flyway baseline-on-migrate](https://documentation.red-gate.com/flyway/reference/configuration/flyway-namespace/flyway-baseline-on-migrate-setting).

The production conversion and initial baseline were operational steps performed outside this repository after verifying the live schema. Future migrations may intentionally change production objects, so the no-op expectation applies to initial creation of objects that already match, not all future schema changes.
...
- Use React Router for client-side navigation. Define `/` for the global home page, `/survivor` for Survivor League, `/madisonsc` for the Madison SC landing page, and the routes specified by each project plan; use router links for internal navigation and keep routing and API fetching explicit. Add broader state/query libraries only when their benefit is clear.
...
- Use the shared `HamburgerMenu` across the home page and all project views, including empty states.
- Provide a clearly labeled hamburger button on both desktop and mobile. It opens a simple navigation panel containing normal links for Home (`/`) and the latest Madison SC picks (`/madisonsc/picks/latest`). Survivor League is intentionally omitted from shared navigation and hides the shared header on `/survivor`; visitors use its direct URL. Add links for future features when they exist.
