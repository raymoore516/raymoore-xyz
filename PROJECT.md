...
This is the implementation plan for AI coding agents. The foundation is implemented: Java 25, Spring Boot 4.1.1 with MVC, Maven Wrapper 3.9.11, local PostgreSQL 15.19 through Compose, Flyway V1, Spring Data JDBC `Contestant` and `Pick` records, public Madison SC summary/latest-week/annual-picks/weekly-picks/contestant-picks APIs, and secret-protected administrator pick submission. The React/TypeScript frontend uses Vite, React Router, a responsive slide-out hamburger menu, a global home page, and Madison SC summary, latest-week, annual, weekly, and contestant-by-team views. Further features remain planned. It defines shared technologies, architecture, and authentication for the personal website. Project-specific business requirements and data models live in `projects/<project>/README.md`. Proposed API contracts and unresolved business rules are labeled explicitly.
...
- Provide a clearly labeled hamburger button on both desktop and mobile. It opens a simple navigation panel containing normal links; start with Home (`/`) and the latest Madison SC picks (`/madisonsc/picks/latest`). Add links for future features when they exist.
